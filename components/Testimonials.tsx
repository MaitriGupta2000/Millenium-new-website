    ...
    <section id="testimonials" ref={ref} className="py-14 lg:py-20 bg-white border-t border-[#E5E5E5]">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        >
          ...
          <div className="inline-flex flex-col items-center gap-1.5 bg-[#1A1A1A] rounded-3xl px-10 py-6 mt-8">
            <div className="flex items-center gap-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  className={`w-5 h-5 ${i < 4 ? "fill-amber-500 text-amber-500" : "fill-amber-500/40 text-amber-500/40"}`}
                />
              ))}
            </div>
            <p className="font-display text-2xl sm:text-3xl text-white">Excellent Rating 4.3/5</p>
            <p className="text-sm text-white/70 font-body">Based on 100+ Reviews</p>
          </div>
        </div>
